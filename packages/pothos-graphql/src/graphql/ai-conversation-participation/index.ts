...
builder.mutationField('addConversationParticipants', (t) =>
  t.prismaField({
    ...
    resolve: async (_query, _source, { conversationId, userIds, assistantIds }) => {
      if (!userIds?.length && !assistantIds?.length) {
        throw new Error('Must provide userIds or assistantIds')
      }
      const existingParticipants = await prisma.aiConversationParticipant.findMany({
      ...
      const newAssistantIds = assistantIds?.filter(
        (assistantId) => !existingParticipants.some((participant) => participant.assistantId === assistantId),
      )
...
builder.mutationField('removeConversationParticipant', (t) =>
  t.prismaField({
    ...
    resolve: async (_query, _source, { participantId }) => {
