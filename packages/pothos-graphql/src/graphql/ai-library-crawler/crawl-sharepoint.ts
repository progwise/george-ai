    ...
    while (processedPages < maxPages) {
      ...
      while (currentUrl && processedPages < maxPages && itemsProcessedInCurrentBatch < TARGET_ITEMS_PER_BATCH) {
        ...
        for (const item of items) {
          ...
          if (!isProcessableFile(fileName)) continue
...
function isProcessableFile(filename: string): boolean {
  const supportedExtensions = [
    ...
    '.make',
    '.cmake',
    // Add file converter supported formats - now supported with transformToMarkdown!
    '.pdf',
    '.docx',
    '.doc',
    '.xlsx',
  ...
  return supportedExtensions.includes(extension) || !filename.includes('.')
}
