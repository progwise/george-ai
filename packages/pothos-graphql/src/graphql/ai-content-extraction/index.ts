...
logger.info('Setting up: ContentExtraction')

export const ExtractionInfo = builder.objectRef<storage.ExtractionMetadata>('ExtractionInfo').implement({
  ...
  fields: (t) => ({
    ...
    extractionHash: t.exposeString('sourceHash', { nullable: false }),
    ...
    attachments: t.field({
      type: [
        builder
          .objectRef<NonNullable<storage.ExtractionMetadata['attachments']>[number]>('ExtractionAttachment')
          .implement({
            fields: (t) => ({
