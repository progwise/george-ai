...
This runs the official Playwright container image (bundled browsers, no need to install anything into the devcontainer itself) with `--network host`, so it reaches your `pnpm dev` services on `localhost:3001`/`localhost:3003` exactly like the native run does, and bind-mounts the whole repo root (not just `e2e-tests`) so it sees your actual test files. The repo root is required because `e2e-tests` is a pnpm workspace package — its `node_modules` are symlinks pointing back up to the root `node_modules`, and mounting `e2e-tests` alone leaves those symlinks broken inside the container. It serves UI Mode over HTTP on port `9323` instead of opening a native window. VS Code will prompt to forward the port automatically (or add it yourself in the **Ports** panel) — open `http://localhost:9323` in your regular browser to get the full UI Mode experience (test tree, watch mode, trace viewer, locator picker).
