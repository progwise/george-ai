...
import { uploadDocumentSource } from '@george-ai/app-domain'
...
export const handleUploadPost = async (httpRequest: Request, httpResponse: Response) => {
  ...
  try {
    const manifest = await uploadDocumentSource(context.workspaceId, {
      libraryId,
      documentId: fileId,
      stream,
    })
