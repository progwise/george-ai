...
def chat():
    ...
    chat_history_objects = [HumanMessage(content=message) if i % 2 == 0 else AIMessage(content=message)
                            for i, message in enumerate(chat_history)]
    
    response = process_chat(chain, question, chat_history_objects)
    chat_history.append(question)
    chat_history.append(response)

    return jsonify({"answer": response, "chat_history": chat_history})
